...
whole_dataset = datasets.ImageFolder(root=r'./train', transform=transform)
...
#加载部分训练集和完整训练集
whole_train_loader = DataLoader(whole_train_set, batch_size=4, shuffle=True, num_workers=0)
...
#加载验证集
dev_loader = DataLoader(dev_set, batch_size=4, shuffle=False, num_workers=0)
...
for para in list(ResNet.parameters())[0:4]:
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        #保存模型
        #torch.save(net, './ResNet_PCB.pt')

#best_acc：epoch=90
#correct: tensor(99, device='cuda:0')
#final result: tensor(0.8250, device='cuda:0')
